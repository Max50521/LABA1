...
Matrix *trans(Matrix *this) {
    Matrix *res = init(this->height, this->width, this->meta);
    for (int i = 0; i < this->height; i++) {
        for (int j = 0; j < this->width; j++) {
...
Matrix *addLinComb(Matrix *a, int row, void *coeffs) {
    Matrix *res = init(a->height, a->width, a->meta);//dimension->height
    for(int i = 0; i < a->height; i++) {//dimension->height j
        for(int j = 0; j < a->width; j++) {//dimension->height i
            if (i != row)
                memcpy(get_ij(i, j, res), get_ij(i, j, a), a->meta->type_m->size);// i->j
